...
> Stream-Processor - Open source tool by AetherCodeHQ

`cli` `data` `golang` `processing` `streaming`

## What is it?

**Stream Processor** is a data tool designed for developers who need fast, reliable, and offline-capable tools. Built with Go for maximum performance and minimal resource usage.
...
```bash
# Basic usage
./Stream-Processor --path ./target

# With options
./Stream-Processor --path ./target --format json --output report.json

# Verbose mode
./Stream-Processor --path ./target --verbose
```
...
### Basic scan
```bash
./Stream-Processor --path ./my-project
```

### JSON report
```bash
./Stream-Processor --path ./my-project --format json --output report.json
```
...
- name: Run Stream Processor
  run: ./Stream-Processor --path . --format json --output report.json
```
